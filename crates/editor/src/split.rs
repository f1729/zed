...
use crate::{
    Autoscroll, DisplayMap, Editor, EditorEvent, SplitSide, ToggleCodeActions, ToggleSoftWrap,
...
impl SplittableEditor {
    ...
    fn split(&mut self, _: &SplitDiff, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let lhs_editor = cx.new(|cx| {
            ...
            editor.split_side = Some(SplitSide::Left);
        ...
        self.rhs_editor.update(cx, |editor, cx| {
            editor.split_side = Some(SplitSide::Right);
            editor.set_delegate_expand_excerpts(true);
